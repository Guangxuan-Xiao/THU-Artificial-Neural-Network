...
if __name__ == "__main__":
    ...
    netG = GAN.get_generator(
        1, args.latent_dim, args.generator_hidden_dim, device)
    ...
    if args.do_train:
        ...
        optimD = optim.Adam(netD.parameters(),
                            lr=args.learning_rate, betas=(args.beta1, 0.999))
        trainer = Trainer(device, netG, netD, optimG, optimD,
    ...
    with torch.no_grad():
        ...
        while samples is None or samples.size(0) < num_samples:
            imgs = netG.forward(torch.randn(
                args.batch_size, netG.latent_dim, 1, 1, device=device))
            if samples is None:
                samples = imgs
            else:
