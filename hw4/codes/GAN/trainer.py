...
class Trainer(object):
    ...
    def train(self, num_training_updates, logging_steps, saving_steps):
        fixed_noise = torch.randn(
            32, self._netG.latent_dim, 1, 1, device=self._device)
        ...
        for i in tqdm(range(num_training_updates), desc='Training'):
            ...
            fake_imgs = self._netG(torch.randn(real_imgs.size(
                0), self._netG.latent_dim, 1, 1, device=self._device))
            errD, errG, D_x, D_G_z1, D_G_z2 = self.train_step(
                real_imgs, fake_imgs, criterion)

            if (i + 1) % logging_steps == 0:
                self._tb_writer.add_scalar(
                    "discriminator_loss", errD, global_step=i)
                self._tb_writer.add_scalar(
                    "generator_loss", errG, global_step=i)
